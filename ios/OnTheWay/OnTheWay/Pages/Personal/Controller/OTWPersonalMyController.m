...
@interface OTWPersonalMyController() <UITableViewDataSource,UITableViewDelegate>
...
@implementation OTWPersonalMyController
-(void)viewDidLoad {
    [super viewDidLoad];
...
-(UITableView*)personalMyTableView{
    if(!_personalMyTableView){
        _personalMyTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 74,SCREEN_WIDTH, SCREEN_HEIGHT-74) style:UITableViewStyleGrouped];
        _personalMyTableView.dataSource = self;
        _personalMyTableView.delegate = self;
        _personalMyTableView.backgroundColor = [UIColor color_f4f4f4];
        // 设置边框颜色
...
-(UIView*)personalMyTableViewHeader{
    if(!_personalMyTableViewHeader){
        //设置header的背景
        _personalMyTableViewHeader=[[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 85)];
        _personalMyTableViewHeader.backgroundColor=[UIColor color_f4f4f4];
...
-(UIImageView*)personalHeadImageView{
    if(!_personalHeadImageView){
        ...
        [_personalHeadImageView setImage: personalHeadImage ];
    }
...
-(UIView*)contentView{
    if(!_contentView){
        _contentView=[[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 85)];
        _personalMyTableViewHeader.backgroundColor=[UIColor whiteColor];
    }
    return _contentView;
}
...
-(UIView*)underLineBottomView{
    if(!_underLineBottomView){
        _underLineBottomView = [[UIView alloc] initWithFrame:CGRectMake(0, 84.5, SCREEN_WIDTH, 0.5)];
