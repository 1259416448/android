...
#define MENU_WIDTH 135  //左侧菜单栏宽度，默认100
...
- (instancetype)initWithFrame:(CGRect)frame WithMenu:(NSArray *)menu andViews:(NSArray *)views{
    if (self = [super init]) {
        ...
        _selectTextColor = [UIColor color_e50834];
        _selectViewColor = [UIColor whiteColor];
...
- (UIView *)rightview{
    if (!_rightview) {
        _rightview = [[UIView alloc] initWithFrame:CGRectMake(MENU_WIDTH, NAVIGATION_HEIGHT+1, FUll_VIEW_WIDTH - MENU_WIDTH + LINEVIEW_WIDTH, FUll_VIEW_HEIGHT - NAVIGATION_HEIGHT)];
    }
    return _rightview;
}
...
- (UIScrollView *)menuView{
    if (!_menuView) {
        _menuView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, navigationHeight+1, MENU_WIDTH, SCREEN_HEIGHT - NAVIGATION_HEIGHT)];
        _menuView.backgroundColor = [UIColor color_f4f4f4];
        ...
        for (int i = 1; i <= menuArray.count; i++) {
            ...
            menuButton.frame = CGRectMake(0, btnHeight * (i - 1) + 1.0, MENU_WIDTH, btnHeight);
            lineX.frame = CGRectMake(0, CGRectGetMaxY(menuButton.frame), MENU_WIDTH, 1.0);
            lineY.frame = CGRectMake(0, btnHeight * (i - 1) + 1.0, 5, btnHeight);
            [lineX setBackgroundColor:[UIColor color_d5d5d5]];
            [lineY setBackgroundColor:[UIColor color_e50834]];
            if (i == 1) {
                [menuButton setTitleColor:[UIColor color_e50834] forState:UIControlStateNormal];
            ...
            [_menuView addSubview:lineX];
            [_menuView addSubview:lineY];
...
//        UIView *lineX = (UIView *)[self viewWithTag:choseTag+100];
//        [lineX setBackgroundColor:[UIColor color_e50834]];
        [lastButton setTitleColor:_textColor forState:UIControlStateNormal];
        [lastButton setBackgroundColor:[UIColor color_f4f4f4]];
        
        CGFloat scroHeight = _menuView.contentSize.height - FUll_VIEW_HEIGHT + NAVIGATION_HEIGHT;
        
        if (menuArray.count > DTScrollTag * 2.0) {
            if (button.tag <= DTScrollTag) {
                [UIView animateWithDuration:ANIMATION_TIME animations:^{
                    [_menuView setContentOffset:CGPointMake(0,0) animated:NO];
                }];
                ...
                [UIView animateWithDuration:ANIMATION_TIME animations:^{
                    [_menuView setContentOffset:CGPointMake(0,blankHeight + 1.0) animated:NO];
                }];
...
- (void)delayChangeTextColor{
        ...
        UIView *lineX = (UIView *)[self viewWithTag:newChoseTag+100];
        [lineX setBackgroundColor:[UIColor color_e50834]];
        [button setTitleColor:_selectTextColor forState:UIControlStateNormal];
