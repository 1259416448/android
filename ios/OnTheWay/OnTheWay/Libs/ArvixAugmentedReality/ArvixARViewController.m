...
@interface ArvixARViewController () <ArvixARTrackingManagerDelegate>

{
    NSArray<ArvixARAnnotation*> *_annotations;
}
...
@implementation ArvixARViewController
...
- (void)initializeInternal
{
    ...
    // Default values
    self.presenter = [[ArvixARPresenter alloc] initWithARViewController:self];
...
- (NSArray*)getRadarSpotsWithARStatus:(ArvixARStatus*)arStatus
{
...
- (int)getARAnnotationXPosition:(ArvixARAnnotation*)annotation withARStatus:(ArvixARStatus*)arStatus
{
 ...
 */
- (void)setAnnotations:(NSArray<ArvixARAnnotation *> *)annotations
{
...
- (void)calculateDistancesForAnnotations
{
    ...
    CLLocation *userLocation = [[CLLocation alloc] initWithLatitude:self.arStatus.userLocation.coordinate.latitude longitude:self.arStatus.userLocation.coordinate.longitude];
...
#pragma mark - Events: ArvixARTrackingManagerDelegate/Display timer
...
//用户位置发生变化，但是没有到达需要重新reload数据 userDistanceFilter
...
//用户位置发生变化，到达了需要reload数据的点。 reloadDistanceFilter
...
- (void)arTrackingManager:(ArvixARTrackingManager *)trackingManager didFailToFindLocationAfter:(NSTimeInterval)elapsedSeconds
{
...
- (void)arTrackingManager:(ArvixARTrackingManager*)trackingManager didFailToLocateWithError:(NSError*)error
{
    //定位错误，最好是子类来实现这个方法
    DLog(@"error %@",error);
...
{
    CaptureSessionResult *reslut = [ArvixCameraView  createCaptureSessionWithMediaType:AVMediaTypeVideo position:AVCaptureDevicePositionBack];
...
- (CLLocation*)centerLocationFromAnnotations:(NSArray<ArvixARAnnotation*>*)annotations
{
...
- (ArvixARTrackingManager*)trackingManager
{
    if (!_trackingManager) {
        _trackingManager = [[ArvixARTrackingManager alloc] init];
    }
...
- (void)setPresenter:(ArvixARPresenter *)presenter
{
    if (presenter) {
...
- (ArvixCameraView*)cameraView
{
    if (!_cameraView) {
        _cameraView = [[ArvixCameraView alloc] init];
    }
...
{
    if (!_radar) {
        _radar = [[ArvixARRadar alloc] initWithFrame:CGRectMake(15, self.view.frame.size.height-95, 80, 80)];
    }
    
    return _radar;
}

- (ArvixARStatus*)arStatus
{
    if (!_arStatus) {
        _arStatus = [[ArvixARStatus alloc] init];
    }
