...
@implementation ArvixCameraView
...
- (void)createSessionAndVideoPreviewLayer
{
    ...
    // ======Capture session
    CaptureSessionResult *result = [ArvixCameraView createCaptureSessionWithMediaType:self.mediaType position:self.devicePosition];
...
+ (CaptureSessionResult*)createCaptureSessionWithMediaType:(NSString*)mediaType position:(AVCaptureDevicePosition)position
{
    ...
    if (captureDevice) {
        ...
        @try {
            ...
            error = [NSError errorWithDomain:@"ArvixCameraView" code:9999 userInfo:dic];
        ...
        if (captureDeviceInput && error == nil) {
            ...
            if ([session canAddInput:captureDeviceInput]) {
                ...
                error = [NSError errorWithDomain:@"ArvixCameraView" code:10002 userInfo:errMsg];
            }
            ...
            error = [NSError errorWithDomain:@"ArvixCameraView" code:10001 userInfo:errMsg];
        }
    } else {
        NSDictionary *errMsg = @{@"description" : @"Back video device not found."};
        error = [NSError errorWithDomain:@"ArvixCameraView" code:10000 userInfo:errMsg];
    }
