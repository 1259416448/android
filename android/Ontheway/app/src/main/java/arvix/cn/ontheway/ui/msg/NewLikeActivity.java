...
public class NewLikeActivity extends BaseActivity implements AdapterView.OnItemClickListener, PullToRefreshBase.OnRefreshListener2<ListView>  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_msg_new_like);
        x.view().inject(self);
        HeaderHolder head=new HeaderHolder();
        head.init(self,"新的赞");
        dataList = new ArrayList();
        initData(true);
        adapter = new NewLikeAdapter(this, dataList);
        ...
        AsyncUtil.goAsync(new Callable<Result<List<ReplyBean>>>() {
            ...
            public void onHandle(Result<List<ReplyBean>> result) {
                if (result.ok()) {
                    ...
                    new AlertDialog.Builder(NewLikeActivity.this).setMessage(result.getErrorMsg()).show();
                }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        UIUtils.toast(NewLikeActivity.this, m.getTitle(), Toast.LENGTH_SHORT);
    }
